...
import requests
from key_events import KeyEvent, KeyEventType
...
# Diffuse un nouveau bloc miné à tous les pairs du réseau
...
# Gère la soumission d'un nouvel événement de clé dans la blockchain
@app.route('/submit_key_event', methods=['POST'])
def submit_key_event():
    event_data = request.get_json()
    required_fields = ["event_type", "key", "email"]

    if not all(k in event_data for k in required_fields):
        return "Invalid event data", 400

    try:
        event_type = KeyEventType(event_data["event_type"])
    except ValueError:
    ...
    event = KeyEvent(
        ...
        event_data["key"],
        event_data["email"],
        event_data.get("public_keys", [])
    )

    transaction = {
        "author": event_data["email"],
...
# Récupère l'historique des événements de clé pour un email donné
@app.route('/key_history/<email>', methods=['GET'])
def get_key_history(email):
    history = KeyManager.get_key_history(email, blockchain)
    return jsonify(history)


# Vérifie si une clé est toujours valide pour un email donné
@app.route('/verify_key', methods=['POST'])
def verify_key():
    data = request.get_json()
    if not data or "key" not in data or "email" not in data:
        return "Invalid request data", 400

    is_valid = KeyManager.is_key_valid(data["key"], data["email"], blockchain)
    return jsonify({"valid": is_valid})


# Réinitialise la blockchain et la liste des pairs à leur état initial
...
def reset():
    ...
    peers = set()
    return jsonify({
        "message": "The blockchain has been reset to its initial state.",
        "chain_length": len(blockchain.chain)
    }), 200
